...
exports = module.exports = {
    ...
    * */
     ...
     * @param {(boolean|dontParseOtherExtensions)=} options.dontParseOtherExtensions  If set to true, files with extensions other than the one specified in the resolverOptions.fileExtension option 
     * (or .md by default) will not be parsed as markdown files, but will be returned as they are. Useful for serving images etc. Forces resolverOptions.useExtensionInUrl to be set to true.
    ...
    middleware: function(options) {
        ...
        if (options.dontParseOtherExtensions)
            server.resolverOptions.useExtensionInUrl = true;

        return function(req, res, next) {
            if (req.method !== 'GET' && !options.handler) {
                next();
                return;
            }
            ...
            server.get(req.path, function(err, result) {
                ...
                var ext = server.resolverOptions.fileExtension || 'md';
                if (ext.indexOf('.') !== 0)
                    ext = '.' + ext;

                if (options.dontParseOtherExtensions && result._file && !_.endsWith(result._file, ext)) {
                    res.sendFile(result._file);
                } else {
